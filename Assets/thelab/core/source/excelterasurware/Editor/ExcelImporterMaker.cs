...
public class ExcelImporterMaker : EditorWindow
{
	...
	private const string templateFileDirectory = @"Assets/thelab/core/source/excelterasurware/Editor/";
	private const string outputFileDirectory = @"Assets/ExcelData/";
	//private const string excelFileDirectory = @"Assets/ExcelData/XLSXTables/";

    void OnGUI()
    {
        GUILayout.Label("importer usage", EditorStyles.boldLabel);
        EditorGUILayout.LabelField("for best structure, put exel tables in <ExcelData/ExcelTable> folder");
        EditorGUILayout.LabelField("repeat \"create\" bellow untill your data is in write into a scriptableobject follow your excel file");
    ...
    static void ExportExcelToAssetbundle()
    {
        foreach (Object obj in Selection.objects)
        {
			...
			using (FileStream stream = File.Open (window.filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                ...
                window.modelName = EditorPrefs.GetString(s_key_prefix + window.fileName + ".modelName", sheet.SheetName + "Model");
    ...
    //[MenuItem("Assets/XLS Import/Create Folders...")]
    //static void ConstructExcelFolders()
    //{
    ...
    void ExportEntity()
    {
		string templateFilePath = (sepalateSheet) ? templateFileDirectory + "EntityTemplate2.txt" : templateFileDirectory + "EntityTemplate.txt";
        ...
        entittyTemplate = entittyTemplate.Replace("$ModelName$", modelName);
        entittyTemplate = entittyTemplate.Replace("$ExcelData$", className);
		
		Directory.CreateDirectory(outputFileDirectory + "Classes/");
		File.WriteAllText(outputFileDirectory + "Classes/" + className + ".cs", entittyTemplate);
    }
	
    void ExportImporter()
    {
		string templateFilePath = (sepalateSheet) ? templateFileDirectory + "ExportTemplate2.txt" : templateFileDirectory + "/ExportTemplate.txt";
        ...
        importerTemplate = importerTemplate.Replace("$ModelName$", modelName);
		...
		Directory.CreateDirectory(outputFileDirectory + "Classes/Editor/");
		File.WriteAllText(outputFileDirectory + "Classes/Editor/" + fileName + "_importer.cs", importerTemplate);
    }
